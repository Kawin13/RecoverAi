...
def test_state_machine_happy_path(client, db_session):
    ...
    res1 = client.post(f"/api/recovery/workflows/{case_id}/step", json={"is_live_demo": False})
    assert res1.status_code == 200
    assert res1.json()["case"]["current_step"] == RecoveryStep.ANALYZED.value

    # 3. Advance Step 2: ANALYZED -> STRATEGY_SELECTED
    res2 = client.post(f"/api/recovery/workflows/{case_id}/step", json={"is_live_demo": False})
    ...
    res3 = client.post(f"/api/recovery/workflows/{case_id}/step", json={"is_live_demo": False})
    ...
    res4 = client.post(f"/api/recovery/workflows/{case_id}/step", json={"is_live_demo": False})
    ...
    res5 = client.post(f"/api/recovery/workflows/{case_id}/step", json={"is_live_demo": False})
    ...
    res6 = client.post(f"/api/recovery/workflows/{case_id}/step", json={"is_live_demo": False})
    assert res6.status_code == 200
    assert res6.json()["case"]["current_step"] == RecoveryStep.WAITING_FOR_CUSTOMER.value

    # 8. Customer pays -> Simulate RECOVERED
    res_rec = client.post(f"/api/recovery/workflows/{case_id}/simulate-outcome", json={"outcome": "RECOVERED"})
...
def test_state_machine_bounded_loop_escalation(client, db_session):
    ...
    res_fail1 = client.post(f"/api/recovery/workflows/{case_id}/simulate-outcome", json={"outcome": "FAILED"})
    ...
    res_fail2 = client.post(f"/api/recovery/workflows/{case_id}/simulate-outcome", json={"outcome": "FAILED"})
    ...
    res_fail3 = client.post(f"/api/recovery/workflows/{case_id}/simulate-outcome", json={"outcome": "FAILED"})
...
def test_audit_records_per_transition(client, db_session):
    order_res = client.post("/api/payments/order", json={
    ...
    # Step 1
    client.post(f"/api/recovery/workflows/{case_id}/step", json={"is_live_demo": False})
    # Step 2
    client.post(f"/api/recovery/workflows/{case_id}/step", json={"is_live_demo": False})
...
def test_executor_payment_link_generation(client, db_session):
    order_res = client.post("/api/payments/order", json={
    ...
    res = client.post(f"/api/recovery/workflows/{case_id}/payment-link", json={"is_live_demo": False})
...
def test_notification_demo_delivery_label(client):
    res = client.get("/api/recovery/notifications")
    assert res.status_code == 200
    receipts = res.json()
...
def test_workflows_list_api(client, db_session):
    res = client.get("/api/recovery/workflows")
    assert res.status_code == 200
    data = res.json()
